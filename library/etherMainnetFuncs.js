...
async function getLivePairData(token0Address, token1Address) {
    var rows = await knex('eth_live')
        ...
        .selectRaw('eth_live.*, CONCAT(YEAR( eth_live.swapAt ), "-", MONTH( eth_live.swapAt ), "-", DAY( eth_live.swapAt ), " ", HOUR(eth_live.swapAt), ":", MINUTE(eth_live.swapAt), ":", SECOND(eth_live.swapAt)) as SWAPAT')
    ...
    for (var i = 0; i < baseTokens.length; i ++) {
        ...
        if (res1.length > 0 && res2.length > 0) {
            for (var j = 0; j < res1.length; j ++) {
                ...
                res1[j].PRICE = res1[j].PRICE * res2[k].PRICE
            }
