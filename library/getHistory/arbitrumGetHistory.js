...
const web3 = new Web3(new Web3.providers.HttpProvider('https://arb-mainnet.g.alchemy.com/v2/u7A_saws6buGy_sCQRUPpgC7WxTTpz7u', options))
...
async function getTokenAndPairData() {
    var res = await knex('arbitrum_tokens').select('*')
    ...
    res = await knex('arbitrum_pairs').select('*')
    ...
    try {
        ...
        for (var i = 0; i < results[0].length; i ++) {
            try {
                ...
                try {
                    await knex('arbitrum_pairs').insert({
                        token0Address: token0Address,
                        token1Address: token1Address,
...
async function getOnePartTransactionHistory(fromBlock, toBlock) {
    try {
            ...
            try {
                ...
                if (!pairsData[pairAddress]) {
                    ...
                    try {
                        ...
                        await knex('arbitrum_pairs').update({
                            blockNumber: block,
                            transactionID: transactionID,
                            lastPrice: Math.abs(swap0 * 1.0 * 10 ** decimals[0] / swap1)
                    ...
                    try {
                        ...
                        await knex('arbitrum_past').insert(data)
                    } catch (err) {
                        myLogger.log(err)
                    }
                } else {
        ...
        if (insertDatas.length) {
            // await knex('arbitrum_past').insert(insertDatas)
        }
    } catch (err) {
        myLogger.log(err)
    }
...
async function writeTransactionHistoryFile(date) {
    var path = "../../database/arbitrum"

    var rows = (await knex.raw('\
        SELECT\
            arbitrum_past.pairAddress AS PAIRADDRESS,\
            CONCAT(YEAR( arbitrum_past.swapAt ), "-", MONTH( arbitrum_past.swapAt ), "-", DAY( arbitrum_past.swapAt )) AS SWAPAT,\
            avg( arbitrum_past.swapPrice ) AS AVGPRICE,\
            max( arbitrum_past.swapPrice ) AS MAXPRICE,\
            min( arbitrum_past.swapPrice ) AS MINPRICE,\
            sum( arbitrum_past.swapAmount0 * ( arbitrum_pairs.baseToken * 2 - 1 ) * ( arbitrum_past.isBuy * - 2 + 1 ) ) AS VOLUME0,\
            sum( arbitrum_past.swapAmount1 * ( arbitrum_pairs.baseToken * - 2 + 1 ) * ( arbitrum_past.isBuy * - 2 + 1 ) ) AS VOLUME1,\
            sum( arbitrum_past.swapAmount0 ) AS TOTALVOLUME0,\
            sum( arbitrum_past.swapAmount1 ) AS TOTALVOLUME1,\
            count( arbitrum_past.swapMaker ) AS SWAPCOUNT \
        FROM\
            arbitrum_past\
            LEFT JOIN arbitrum_pairs ON arbitrum_pairs.pairAddress = arbitrum_past.pairAddress \
        WHERE\
            arbitrum_past.swapAt<"' + date + ' ' + '00:00:00' + '"\
        GROUP BY\
            arbitrum_past.pairAddress,\
            DATE( arbitrum_past.swapAt ) \
        ORDER BY\
            DATE( arbitrum_past.swapAt)'))[0]
    ...
    rows = (await knex.raw('select CONCAT(YEAR( arbitrum_past.swapAt ), "-", MONTH( arbitrum_past.swapAt ), "-", DAY( arbitrum_past.swapAt )) AS SWAPAT, swapMaker as SWAPMAKER, pairAddress from arbitrum_past where arbitrum_past.swapAt<"' + date + ' ' + '00:00:00' + '" order by swapAt'))[0]
    ...
    await knex('arbitrum_past').where('swapAt', '<', date + ' ' + '00:00:00').delete()
...
.then(res => {
    ...
    myLogger.log(FROMBLOCK + '~' + TOBLOCK + ' arbitrum_past')
