...
async function getOnePartTransactionHistory(fromBlock, toBlock) {
    try {
            ...
            try {
                ...
                if (!pairsData[pairAddress]) {
                    ...
                    try {
                        ...
                        await knex('eth_past14').insert(data)
                    } catch (err) {
                        myLogger.log(err)
                    }
                } else {
        ...
        if (insertDatas.length) {
            // await knex('eth_past14').insert(insertDatas)
        }
    } catch (err) {
        myLogger.log(err)
    }
...
async function writeTransactionHistoryFile(date) {
    ...
    var rows = (await knex.raw('\
        SELECT\
            eth_past14.pairAddress AS PAIRADDRESS,\
            CONCAT(YEAR( eth_past14.swapAt ), "-", MONTH( eth_past14.swapAt ), "-", DAY( eth_past14.swapAt )) AS SWAPAT,\
            avg( eth_past14.swapPrice ) AS AVGPRICE,\
            max( eth_past14.swapPrice ) AS MAXPRICE,\
            min( eth_past14.swapPrice ) AS MINPRICE,\
            sum( eth_past14.swapAmount0 * ( eth_pairs.baseToken * 2 - 1 ) * ( eth_past14.isBuy * - 2 + 1 ) ) AS VOLUME0,\
            sum( eth_past14.swapAmount1 * ( eth_pairs.baseToken * - 2 + 1 ) * ( eth_past14.isBuy * - 2 + 1 ) ) AS VOLUME1,\
            sum( eth_past14.swapAmount0 ) AS TOTALVOLUME0,\
            sum( eth_past14.swapAmount1 ) AS TOTALVOLUME1,\
            count( eth_past14.swapMaker ) AS SWAPCOUNT \
        FROM\
            eth_past14\
            LEFT JOIN eth_pairs ON eth_pairs.pairAddress = eth_past14.pairAddress \
        WHERE\
            eth_past14.swapAt<"' + date + ' ' + '00:00:00' + '"\
        GROUP BY\
            eth_past14.pairAddress,\
            DATE( eth_past14.swapAt ) \
        ORDER BY\
            DATE( eth_past14.swapAt)'))[0]
    ...
    rows = (await knex.raw('select CONCAT(YEAR( eth_past14.swapAt ), "-", MONTH( eth_past14.swapAt ), "-", DAY( eth_past14.swapAt )) AS SWAPAT, swapMaker as SWAPMAKER, pairAddress from eth_past14 where eth_past14.swapAt<"' + date + ' ' + '00:00:00' + '" order by swapAt'))[0]
    ...
    await knex('eth_past14').where('swapAt', '<', date + ' ' + '00:00:00').delete()
...
.then(res => {
    ...
    myLogger.log(FROMBLOCK + '~' + TOBLOCK + ' eth_past14')
