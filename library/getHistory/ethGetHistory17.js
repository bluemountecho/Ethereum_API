...
async function getOnePartTransactionHistory(fromBlock, toBlock) {
    try {
            ...
            try {
                ...
                if (!pairsData[pairAddress]) {
                    ...
                    try {
                        ...
                        await knex('eth_past17').insert(data)
                    } catch (err) {
                        myLogger.log(err)
                    }
                } else {
        ...
        if (insertDatas.length) {
            // await knex('eth_past17').insert(insertDatas)
        }
    } catch (err) {
        myLogger.log(err)
    }
...
async function writeTransactionHistoryFile(date) {
    ...
    var rows = (await knex.raw('\
        SELECT\
            eth_past17.pairAddress AS PAIRADDRESS,\
            CONCAT(YEAR( eth_past17.swapAt ), "-", MONTH( eth_past17.swapAt ), "-", DAY( eth_past17.swapAt )) AS SWAPAT,\
            avg( eth_past17.swapPrice ) AS AVGPRICE,\
            max( eth_past17.swapPrice ) AS MAXPRICE,\
            min( eth_past17.swapPrice ) AS MINPRICE,\
            sum( eth_past17.swapAmount0 * ( eth_pairs.baseToken * 2 - 1 ) * ( eth_past17.isBuy * - 2 + 1 ) ) AS VOLUME0,\
            sum( eth_past17.swapAmount1 * ( eth_pairs.baseToken * - 2 + 1 ) * ( eth_past17.isBuy * - 2 + 1 ) ) AS VOLUME1,\
            sum( eth_past17.swapAmount0 ) AS TOTALVOLUME0,\
            sum( eth_past17.swapAmount1 ) AS TOTALVOLUME1,\
            count( eth_past17.swapMaker ) AS SWAPCOUNT \
        FROM\
            eth_past17\
            LEFT JOIN eth_pairs ON eth_pairs.pairAddress = eth_past17.pairAddress \
        WHERE\
            eth_past17.swapAt<"' + date + ' ' + '00:00:00' + '"\
        GROUP BY\
            eth_past17.pairAddress,\
            DATE( eth_past17.swapAt ) \
        ORDER BY\
            DATE( eth_past17.swapAt)'))[0]
    ...
    rows = (await knex.raw('select CONCAT(YEAR( eth_past17.swapAt ), "-", MONTH( eth_past17.swapAt ), "-", DAY( eth_past17.swapAt )) AS SWAPAT, swapMaker as SWAPMAKER, pairAddress from eth_past17 where eth_past17.swapAt<"' + date + ' ' + '00:00:00' + '" order by swapAt'))[0]
    ...
    await knex('eth_past17').where('swapAt', '<', date + ' ' + '00:00:00').delete()
...
.then(res => {
    ...
    myLogger.log(FROMBLOCK + '~' + TOBLOCK + ' eth_past17')
